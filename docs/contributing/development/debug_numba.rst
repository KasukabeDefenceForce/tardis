...
`tardis.scripts.debug`. Currenly, these include the abilty to run TARDIS
in a single-packet mode.
`tardis_example_single.yml` is the configuration file that is used to set up the
single-pcket TARDIS run; `run_numba_single.py` is thePython script that runs
...
Note that this metod is EXPERIMENTAL.
